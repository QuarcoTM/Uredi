(function(){
  ...
  const BONUS_TOP7_MIGRATION_KEY='marketBonusTop7MigrationV252';
  ...
  function migrateEarlyBonusToTop7(config){
    const c=normalize(config);
    if(localStorage.getItem(BONUS_TOP7_MIGRATION_KEY)==='1')return c;

    // v2.52: earlier local builds could retain an old "1 × Изкачи" campaign
    // in browser storage. The agreed FREE BETA reward is 1 × TOP for 7 days.
    if(c.bonusCampaign){
      c.bonusCampaign.id='early-500-top7';
      c.bonusCampaign.creditProductId='top7';
      c.bonusCampaign.qty=1;
      c.bonusCampaign.maxVerifiedUsers=Number(c.bonusCampaign.maxVerifiedUsers||500);
    }

    const grants=read(BONUS_GRANTS_KEY,{});
    let grantsChanged=false;
    Object.keys(grants).forEach(id=>{
      const grant=grants[id];
      if(grant&&grant.campaignId==='early-500-top7'&&grant.productId==='bump'){
        grant.productId='top7';
        grantsChanged=true;
      }
    });
    if(grantsChanged)write(BONUS_GRANTS_KEY,grants);

    write(CONFIG_KEY,c);
    localStorage.setItem(BONUS_TOP7_MIGRATION_KEY,'1');
    return c;
  }

  function getConfig(){
    const stored=localStorage.getItem(CONFIG_KEY);
    if(stored)return migrateEarlyBonusToTop7(read(CONFIG_KEY,DEFAULTS));
