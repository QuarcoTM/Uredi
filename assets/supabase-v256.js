(()=>{
  ...
  const client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey,{
    ...
    global:{headers:{'X-Client-Info':'uredi-web/2.56'}}
  });
  ...
  async function initRegistration(){
    ...
    btn.addEventListener('click',async()=>{
      ...
      localStorage.setItem('marketPendingVerifyEmailV256',email);
  ...
  async function initVerifyEmail(){
    ...
    const refresh=async()=>{
      ...
      if(session?.user?.email_confirmed_at){
        ...
        localStorage.removeItem('marketPendingVerifyEmailV256');
    ...
    resend?.addEventListener('click',async()=>{
      const email=localStorage.getItem('marketPendingVerifyEmailV256')||'';
