(()=>{
  ...
  const client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey,{
    ...
    global:{headers:{'X-Client-Info':'uredi-web/2.69'}}
  });
  ...
  async function initSupabasePostAd(session,account){
    ...
    publish.addEventListener('click',async e=>{
      ...
      try{
        ...
        if(/no unique or exclusion constraint matching the ON CONFLICT/i.test(technical))msg=`Supabase има липсващо UNIQUE правило при ${publishStage}. Пусни v2.72 SQL поправката веднъж.`;
