(()=>{
  ...
  const client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey,{
    ...
    global:{headers:{'X-Client-Info':'uredi-web/2.73'}}
  });
    ...
    publish.addEventListener('click',async e=>{
      ...
      try{
        ...
        for(let i=0;i<all.length;i++){
          ...
          if(up?.error)throw up.error;
          uploaded.push(path);
          publishStage=`запис на снимка ${i+1}/${all.length}`;
        ...
        if(/no unique or exclusion constraint matching the ON CONFLICT/i.test(technical))msg=`Supabase върна технически ON CONFLICT проблем при ${publishStage}. v2.73 вече опитва и signed upload; ако виждаш това съобщение, прати екрана и ще знаем точния етап.`;
