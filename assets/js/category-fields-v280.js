/* Uredi v2.78 — category-specific listing characteristics.
...
(() => {
  ...
  function cleanStoredValue(value,def){
  ...
  function valueFromMap(values,def){
    if(!values||typeof values!=='object')return '';
    if(Object.prototype.hasOwnProperty.call(values,def.key))return values[def.key];
    if(Object.prototype.hasOwnProperty.call(values,def.label))return values[def.label];
    const wanted=new Set([norm(def.key),norm(def.label)]);
  ...
  function captureValues(host,defs){
    const current={};
    (defs||[]).forEach(def=>{
      const ctrl=host.querySelector(`[data-category-spec="${CSS.escape(def.key)}"]`);
      if(!ctrl)return;
      current[def.key]=ctrl.value;
    });
    return current;
  }

  function applyValues(host,defs,values){
    (defs||[]).forEach(def=>{
      const ctrl=host.querySelector(`[data-category-spec="${CSS.escape(def.key)}"]`);
      if(!ctrl)return;
      const raw=valueFromMap(values,def);
      if(raw===undefined||raw===null||String(raw).trim()==='')return;
      const value=cleanStoredValue(raw,def);
      if(ctrl.tagName==='SELECT'){
        const option=[...ctrl.options].find(opt=>norm(opt.value||opt.textContent)===norm(value));
        if(option)ctrl.value=option.value;
      }else{
        ctrl.value=value;
      }
    });
  ...
  function render(){
    ...
    const previous=host.dataset.category===category?captureValues(host,defs):{};
    ...
    if(category===prefillCategory)applyValues(host,defs,prefillValues);
    applyValues(host,defs,previous);
    try{window.UrediFormUX?.refreshCounters?.()}catch{}
  }

  function setValues(category,values){
    prefillCategory=String(category||'');
    prefillValues=values&&typeof values==='object'?{...values}:{};
    const select=q('[data-ad-category]');
    if(select&&prefillCategory)select.value=prefillCategory;
    render();
  }

  function init(){
    window.UrediCategoryFields={render,setValues,configs};
    const bind=()=>{
      const select=q('[data-ad-category]');
      const host=q('[data-category-fields]');
      if(!select||!host)return;
