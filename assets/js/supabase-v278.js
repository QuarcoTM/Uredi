(()=>{
  ...
  const client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey,{
    ...
    global:{headers:{'X-Client-Info':'uredi-web/2.74'}}
  });
  ...
  async function initRealListingDetail(session){
    ...
    const phoneButton=phone?`<a class="secondary-btn icon-action-button phone-action-button" href="${esc(phoneHref)}"><span class="icon-action-label">Обади се</span></a>`:'';
    const chatOnly=!phone?`<div class="real-chat-only-note"><span class="real-chat-only-icon" aria-hidden="true">●</span><span>Контактът е достъпен само чрез вътрешния чат.</span></div>`:'';
    ...
    main.innerHTML=`<div class="container real-listing-detail"><div class="breadcrumb"><a href="index.html">Начало</a><span>/</span><a href="listings.html?category=${encodeURIComponent(fields.category)}">${esc(fields.category)}</a><span>/</span><span>${esc(row.title||'Обява')}</span></div>
      ...
      <div class="listing-action-panel"><div class="listing-action-panel-head">Действия по обявата</div><div class="detail-actions"><a class="${messageClass}" href="messages.html?listing=${encodeURIComponent(id)}&seller=${encodeURIComponent(row.seller_id||'')}">Съобщение</a>${phoneButton}<button class="secondary-btn" data-favorite="${esc(id)}" type="button">Запази обявата</button><button class="secondary-btn" data-real-share type="button">Сподели обявата</button></div>${chatOnly}</div></section>
      ...
      <div class="seller-card"><div class="seller-head"><div class="avatar">${esc(v260Initials(seller))}</div><div><strong>${esc(seller)}</strong><span class="seller-type-inline"><span>${dealer?'Търговец':'Частно лице'}${profile.city?' · '+esc(profile.city):''}</span></span></div></div>${phone?`<a class="secondary-btn seller-phone-bottom phone-action-button" href="${esc(phoneHref)}">Обади се</a>`:''}<a class="secondary-btn" href="seller.html?id=${encodeURIComponent(row.seller_id||'')}" style="width:100%;margin-top:10px">Виж профила</a><div class="seller-secondary-actions"><a class="secondary-btn listing-report-button" href="report.html?type=listing&listing=${encodeURIComponent(id)}">Докладвай обявата</a></div></div></aside></div></div>`;
  ...
  async function initRealEditAd(session){
    ...
    if(window.UrediCategoryFields?.setValues){
      window.UrediCategoryFields.setValues(fields.category,fields.specs||{});
    ...
    setTimeout(()=>{
      try{
        if(window.UrediCategoryFields?.setValues){
          ...
          if(!hasUserInput)window.UrediCategoryFields.setValues(fields.category,fields.specs||{});
        }else fillSpecs();
