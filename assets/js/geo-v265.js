(function(){
  ...
  function closeConsent(){
    const modal=$('[data-geo-consent]');
    if(!modal)return;
    modal.classList.remove('is-open');
    modal.setAttribute('aria-hidden','true');
    document.documentElement.classList.remove('geo-modal-open');
  }

  function ensureConsent(){
    let modal=$('[data-geo-consent]');
    if(modal)return modal;
    modal=document.createElement('div');
    modal.className='geo-consent-backdrop';
    modal.setAttribute('data-geo-consent','');
    modal.setAttribute('aria-hidden','true');
    modal.innerHTML=`
      <div class="geo-consent-card" role="dialog" aria-modal="true" aria-labelledby="geo-consent-title">
        <button class="geo-consent-close" type="button" aria-label="Затвори" data-geo-close>×</button>
        <span class="geo-consent-icon" aria-hidden="true">⌖</span>
        <h2 id="geo-consent-title">Използвай текущото ми място</h2>
        <p>Ще използваме местоположението само за филтриране на обявите по разстояние. Браузърът ще поиска разрешение.</p>
        <div class="geo-consent-actions">
          <button class="secondary-btn" type="button" data-geo-close>Отказ</button>
          <button class="primary-btn" type="button" data-geo-allow>Продължи</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
    modal.addEventListener('click',e=>{
      if(e.target===modal||e.target.closest('[data-geo-close]'))closeConsent();
      const allow=e.target.closest('[data-geo-allow]');
      if(allow){
        closeConsent();
        setTimeout(()=>requestLocation(),60);
      }
    });
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal.classList.contains('is-open'))closeConsent()});
    return modal;
  }

  function openConsent(){
    const modal=ensureConsent();
    modal.classList.add('is-open');
    modal.setAttribute('aria-hidden','false');
    document.documentElement.classList.add('geo-modal-open');
    setTimeout(()=>modal.querySelector('[data-geo-allow]')?.focus(),0);
  }

  function requestLocation(){
    const b=button();if(!b)return;
    b.disabled=true;b.classList.add('geo-location-busy');b.textContent='Определям…';
    if(hint())hint().textContent='Определям текущото ти местоположение…';
    ...
    navigator.geolocation.getCurrentPosition(pos=>{
      ...
      const msg=err?.code===1?'Местоположението е блокирано. Разреши го от иконата до адреса на сайта и опитай пак.':err?.code===3?'Не успях да определя местоположението навреме. Опитай пак.':'Не успях да определя местоположението. Можеш да избереш град ръчно.';
      fail(msg);
  ...
  async function locate(){
    ...
    try{
      if(navigator.permissions?.query){
        ...
        if(permission.state==='denied')return fail('Местоположението е блокирано. Разреши го от иконата до адреса на сайта и опитай пак.');
      }
    }catch(e){}
    openConsent();
  }
