  ...
  async function initSupabasePostAd(session,account){
    ...
    publish.addEventListener('click',async e=>{
      ...
      try{
        ...
        if(/no unique or exclusion constraint matching the ON CONFLICT/i.test(technical))msg=`Supabase върна технически ON CONFLICT проблем при ${publishStage}. v2.74 вече опитва и signed upload; ако виждаш това съобщение, прати екрана и ще знаем точния етап.`;
